...
class NewColorDialog(NewColorPicker):
    def __init__(self, parent=None):
        NewColorPicker.__init__(self, width=250, startupcolor=[
                                0, 255, 255], parent=parent)
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for cuelist in GlobalVar.cuelist_dict.keys():
            ...
            getattr(self, "{0:s}_cue_thread".format(cuelist)
                    ).lampset.connect(self.lampset_relay)
            getattr(self, "{0:s}_cue_thread".format(
                cuelist)).set_cuelist(str(cuelist))
    ...
    def closeEvent(self, event):
        close = QMessageBox.question(
            self, "QUIT", "Are you sure want to quit?", QMessageBox.Yes | QMessageBox.No, QMessageBox.No)
    ...
    def exec_program_line(self):
        ...
        try:
            if "/" in text:
                ...
                number = nums.split("/")
                numbers = "+".join([str(x)
                                    for x in range(int(number[0]), int(number[1])+1)])
        ...
        self.statusbarhistory.setText(
            "+".join(GlobalVar.in_use_programmer.keys()))
        self.statuslinebar.clear()
    ...
    def create_output(self):
        ...
        for num in list(GlobalVar.nr_to_typ.keys()):
            ...
            if not GlobalVar.typ_to_func[GlobalVar.nr_to_typ[num]]['Color'] == False:
                setattr(self, "output_{0:d}_Color".format(
                    num), QLabel('(0,0,0)'))
                self.output_layout.addWidget(
    ...
    def set_output(self, num, setting, value):
        try:
            if any(setting == x for x in ["Red", "Green", "Blue"]):
                self.text = getattr(
                    self, "output_{0:d}_Color".format(num)).text()
                ...
                getattr(self, "output_{0:d}_Color".format(
                    num)).setText("("+",".join(text)+")")
            else:
                getattr(self, "output_{0:d}_{1:s}".format(
                    num, setting)).setText(str(value))
        except:
            pass
    ...
    def create_master_fader(self):
        ...
        width_edit.returnPressed.connect(lambda: self.master_slid.resize(
            QSize(int(width_edit.text()), self.master_slid.size().height())))
    ...
    def create_keys(self):
        ...
        for row in range(GlobalVar.rows):
            for col in range(GlobalVar.cols):
                ...
                getattr(self, "key"+str(num)
                        ).pressed.connect(partial(self.key_pressed, str(num)))
    ...
    def chase_send(self):
        for i in [110, 111, 113, 115, 114, 112]:
            ...
            color_tup = colorsys.hsv_to_rgb(
                abs(math.sin(self.freq*time.time()+(self.add_fac*(i-110)))), 1, 1)  # 0))),1,1)
    ...
    def create_sub_area(self, name, title, wid_list, width=None):
        ...
        for wid in wid_list:
            if len(wid) == 3:
                ...
                wid.append(1)
            getattr(
                self, name+"_layout").addWidget(wid[0], wid[1], wid[2], wid[3], wid[4])
        if width:
        ...
        getattr(
            self, name+"act").triggered.connect(lambda: getattr(self, name+"_sub").show())
    ...
    def create_fader(self, name, label, liste, start, start1, fad_max=100):
        ...
        getattr(
            self, name+"_slid").valueChanged.connect(getattr(self, name+"_slid_fader"))
